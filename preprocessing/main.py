...
from alignment_pre import (
	apply_shift_to_static,
	clip_by_mileage_range,
	estimate_shift_by_superelevation,
	get_alignment_range,
	plot_alignment_before_after,
	save_aligned_results,
)
from dynamic_pre import process_dynamic_file
...
def run_preprocessing_pipeline() -> dict:
	...
	# 4) 基于超高(静检实测水平)进行动静数据里程对齐 + 绘图
	shift_km = estimate_shift_by_superelevation(dynamic_df, static_df)
	static_aligned = apply_shift_to_static(static_df, shift_km)

	# 4.1) 按最小公共里程范围裁剪（默认即静检范围与动检交集）
	x0, x1 = get_alignment_range(dynamic_df, static_aligned, prefer_static=True)
	dynamic_aligned = clip_by_mileage_range(dynamic_df, x0, x1, mileage_col="里程")
	...
	align_plot = base / "静检数据" / "呼局" / "20210416" / "原始文件" / "绘图检测" / "动静检对齐前后_七通道.png"
	plot_alignment_before_after(
		dynamic_df=dynamic_aligned,
		static_before=static_df,
		static_after=static_aligned,
		out_file=align_plot,
	)
